...
function web_pengabdian_register_cpts() {
	...
	register_taxonomy( 'kategori_faq', 'faq', array(
		'labels' => array(
			'name' => __( 'Kategori FAQ', 'twentytwentyfour' ),
			'singular_name' => __( 'Kategori FAQ', 'twentytwentyfour' ),
			'search_items' => __( 'Cari Kategori FAQ', 'twentytwentyfour' ),
			'all_items' => __( 'Semua Kategori FAQ', 'twentytwentyfour' ),
			'edit_item' => __( 'Edit Kategori FAQ', 'twentytwentyfour' ),
			'update_item' => __( 'Update Kategori FAQ', 'twentytwentyfour' ),
		...
		'query_var' => true,
		'show_in_rest' => true,
		'rewrite' => array( 'slug' => 'kategori-faq' ),
	) );
...
function web_pengabdian_faq_accordion_shortcode() {
	...
	$output = '<section class="wp-faq-section-wrapper reveal-on-scroll" style="background: #eef4ff; padding: 75px 24px;">
		<div style="max-width: 860px; margin: 0 auto;">
			<div style="text-align: center; margin-bottom: 45px;">
				<div style="display: inline-flex; align-items: center; justify-content: center; gap: 8px; padding: 6px 18px; background: #dbeafe; color: #0c54a3; border-radius: 99px; font-size: 13px; font-weight: 700; margin-bottom: 16px; letter-spacing: 0.4px;">
					<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" style="display: block; flex-shrink: 0;"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
					<span style="display: inline-block; line-height: 1.2;">FAQ &amp; PANDUAN WEBINAR</span>
				</div>
				<h2 style="font-size: 32px; font-weight: 800; color: #0f2340; margin: 0 0 10px 0; letter-spacing: -0.4px;">Pertanyaan yang Sering Diajukan</h2>
				<p style="color: #64748b; font-size: 15px; margin: 0;">Rangkuman FAQ dan diskusi penting dari seluruh seri webinar pelatihan pengabdian UNS x Perma UTLLN Jepang.</p>
			</div>';
	...
	if ( $query->have_posts() ) {
		...
		while ( $query->have_posts() ) {
			$query->the_post();
			$post_id = get_the_ID();
			$terms = get_the_terms( $post_id, 'kategori_faq' );
			$cat_name = ($terms && !is_wp_error($terms)) ? $terms[0]->name : 'Umum / Lainnya';
		...
		foreach ( $grouped_faqs as $cat_title => $faq_items ) {
			$output .= '
			<div class="wp-faq-category-group" style="margin-bottom: 45px; background: #ffffff; padding: 28px 26px; border-radius: 16px; box-shadow: 0 4px 20px rgba(12, 84, 163, 0.06); border: 1px solid rgba(12, 84, 163, 0.1);">
				<div style="margin-bottom: 22px; padding-bottom: 14px; border-bottom: 2px solid #e2e8f0;">
					<div style="display: inline-flex; align-items: center; gap: 6px; padding: 4px 12px; background: #0c54a3; color: #ffffff; border-radius: 6px; font-size: 12px; font-weight: 700; margin-bottom: 8px; text-transform: uppercase; letter-spacing: 0.5px;">
						' . esc_html( $cat_title ) . '
					</div>
				</div>
				<div class="wp-faq-list" style="display: flex; flex-direction: column; gap: 12px;">';
		...
		$output .= '<p style="text-align: center; color: #64748b; font-size: 15px;">Belum ada FAQ yang diterbitkan.</p>';
	}
...
/**
 * Ensure Query Loop blocks for 'anggota' (Pemateri) display all posts instead of being restricted by block default limit of 4.
 */
add_filter( 'query_loop_block_query_vars', function( $query, $block ) {
	if ( isset( $query['post_type'] ) && $query['post_type'] === 'anggota' ) {
...
add_action( 'pre_get_posts', function( $query ) {
	if ( ! is_admin() && $query->get( 'post_type' ) === 'anggota' ) {
		if ( (int) $query->get( 'posts_per_page' ) < 10 ) {
...
add_filter( 'manage_faq_posts_columns', function( $columns ) {
	$new_columns = array(
		...
		'taxonomy-kategori_faq' => __( 'Kategori', 'twentytwentyfour' ),
...
/**
 * Add Category Filter Dropdown to FAQ Admin List Table
 */
	...
	if ( $post_type === 'faq' ) {
		...
		if ( $info_taxonomy ) {
			wp_dropdown_categories( array(
				'show_option_all' => sprintf( __( 'Semua %s', 'twentytwentyfour' ), $info_taxonomy->label ),
				'taxonomy'        => $taxonomy,
				'name'            => $taxonomy,
				'orderby'         => 'name',
				'selected'        => $selected,
